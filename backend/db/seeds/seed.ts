...
const routeTemplates = [
  { name: 'Apapa Port to Ibadan', originName: 'Apapa Port Gate 3', destinationName: 'Ibadan Distribution Centre' },
  { name: 'Lagos Island to Ota', originName: 'Marina CMS', destinationName: 'Ota Industrial Park' },
  { name: 'Ikorodu to Sagamu', originName: 'Ikorodu Garage', destinationName: 'Sagamu Interchange' }
];

const firstNames = ['Adebayo', 'Chinedu', 'Kehinde', 'Ngozi', 'Ifeanyi', 'Temitope', 'Uche', 'Bolanle', 'Emeka', 'Kelechi'];
const lastNames = ['Afolabi', 'Okonkwo', 'Adeyemi', 'Nwankwo', 'Balogun', 'Obi', 'Akinola', 'Eze'];

async function seed(): Promise<void> {
  for (const name of tenantNames) {
    const [tenant] = await db.insert(tenants).values({ name, maxVehicles: 50 }).returning();
    if (!tenant) {
      continue;
    }
    ...
    for (let i = 0; i < 3; i += 1) {
      await db.insert(users).values({
        ...
        passwordHash: '$2a$12$Z0ruH2pH92f5weX5fM8f0u0ckM4rWcBv3JUxJ8Q8QkMAnmMyN0pq2',
        role: 'fleet_manager',
    ...
    for (const template of routeTemplates) {
      const [route] = await db.insert(routes).values({
        ...
        distanceKm: String(faker.number.float({ min: 25, max: 140, fractionDigits: 2 }))
      }).returning();
      if (route) {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      const [vehicle] = await db.insert(vehicles).values({
        ...
        fuelTankCapacityLitres: String(faker.number.float({ min: 60, max: 450, fractionDigits: 2 })),
    ...
    for (let i = 0; i < 50; i += 1) {
      ...
      const [trip] = await db.insert(trips).values({
        ...
        endTime: end,
        status: 'completed',
        totalDistanceKm: String(faker.number.float({ min: 8, max: 220, fractionDigits: 2 })),
        totalFuelUsedLitres: String(faker.number.float({ min: 2, max: 90, fractionDigits: 2 })),
        complianceScore: String(faker.number.float({ min: 65, max: 100, fractionDigits: 2 }))
      }).returning();
      ...
      for (let point = 0; point < 10; point += 1) {
        await db.insert(gpsEvents).values({
          ...
          speedKph: String(faker.number.float({ min: 0, max: 95, fractionDigits: 2 })),
          headingDeg: faker.number.int({ min: 0, max: 359 }),
          altitudeM: String(faker.number.float({ min: 2, max: 120, fractionDigits: 2 })),
    ...
    for (let i = 0; i < 20; i += 1) {
      await db.insert(alerts).values({
        ...
        vehicleId: faker.helpers.arrayElement(tenantVehicles),
        driverId: faker.helpers.arrayElement(tenantDrivers),
        type: faker.helpers.arrayElement(['fuel_anomaly', 'idle_excess', 'private_use', 'geofence_breach']),
