...
export function parsePagination(params: PaginationParams): {
  ...
  const limit = Math.min(
    Math.max(params.limit ?? DEFAULT_CURSOR_LIMIT, 1),
    MAX_CURSOR_LIMIT,
  )
  if (!params.cursor) {
...
export function buildCursorWhere(
  ...
  return or(
    ...
    and(
      sql`${createdAtColumn} = ${new Date(cursor.createdAt).toISOString()}`,
...
export const orderByNewest = <T extends { createdAt: Date; id: string }>(
  rows: T[],
): T[] => {
