...
import { z } from 'zod';
import { ALERT_CHANNEL_PREFIX, ALERT_DEDUP_PREFIX, IDLE_THRESHOLD_MINUTES_DEFAULT } from '../config/constants.js';
...
type AlertType = 'fuel_anomaly' | 'geofence_breach' | 'idle_excess' | 'private_use';
  ...
  while (!signal.aborted) {
    ...
    await Promise.all(
      messages.map(async (message) => {
        ...
        const [tenant] = await db.select().from(tenants).where(eq(tenants.id, payload.tenantId)).limit(1);
        const [vehicle] = await db
          .select()
          .from(vehicles)
          .where(and(eq(vehicles.id, payload.vehicleId), eq(vehicles.tenantId, payload.tenantId)))
          .limit(1);
        ...
        const [activeTrip] = await db
          .select()
          .from(trips)
          .where(and(eq(trips.tenantId, payload.tenantId), eq(trips.vehicleId, payload.vehicleId), eq(trips.status, 'active')))
          .limit(1);

        const [route] = activeTrip?.routeId
          ? await db.select().from(routes).where(and(eq(routes.id, activeTrip.routeId), eq(routes.tenantId, payload.tenantId))).limit(1)
          : [];
        ...
        const geofenceDetected = route ? !(await isWithinCorridor(db, payload.lat, payload.lon, route.id)) : false;

        const candidates: AlertCandidate[] = [];
        if (idleDetected) {
          candidates.push({ type: 'idle_excess', severity: 'warning', description: 'Sustained idle detected' });
        }
        if (fuelDetected) {
          candidates.push({ type: 'fuel_anomaly', severity: 'critical', description: 'Abnormal fuel consumption detected' });
        }
        if (privateUseDetected) {
          candidates.push({ type: 'private_use', severity: 'warning', description: 'Movement outside operating hours' });
        }
        if (geofenceDetected) {
          candidates.push({ type: 'geofence_breach', severity: 'critical', description: 'Vehicle outside approved corridor' });
        }
