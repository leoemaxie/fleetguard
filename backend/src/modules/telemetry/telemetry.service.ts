...
export async function getVehicleGps(
  ...
  try {
    const filters = [
      ...
      eq(gpsEvents.vehicleId, vehicleId),
    ]
    if (query.from && query.to) {
      filters.push(
        between(gpsEvents.ts, new Date(query.from), new Date(query.to)),
      )
    }
...
export async function getVehicleFuel(
  ...
  try {
    const filters = [
      ...
      eq(fuelEvents.vehicleId, vehicleId),
    ]
    if (query.from && query.to) {
      filters.push(
        between(fuelEvents.ts, new Date(query.from), new Date(query.to)),
      )
    }
    ...
    throw new AppError(
      ErrorCode.INTERNAL_ERROR,
      'Failed to fetch fuel telemetry',
      500,
      error,
    )
  }
