...
import type { CreateVehicleSchema, UpdateVehicleSchema } from './vehicles.schema.js';
...
export async function listVehicles(
  app: FastifyInstance,
  tenantId: string,
  query: { cursor?: string; limit?: number; status?: 'active' | 'inactive' | 'maintenance'; driverId?: string }
) {
  try {
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to list vehicles', 500, error);
  }
}
...
export async function getVehicle(app: FastifyInstance, tenantId: string, vehicleId: string) {
  try {
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to get vehicle', 500, error);
  }
}
...
export async function updateVehicle(app: FastifyInstance, tenantId: string, vehicleId: string, payload: UpdateVehicleInput) {
  try {
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to update vehicle', 500, error);
  }
}

export async function softDeleteVehicle(app: FastifyInstance, tenantId: string, vehicleId: string) {
  return updateVehicle(app, tenantId, vehicleId, { status: 'inactive' });
}

export async function getVehicleLiveSnapshot(app: FastifyInstance, tenantId: string, vehicleId: string) {
  try {
    const value = await app.redis.get(`${LIVE_CACHE_PREFIX}:${tenantId}:${vehicleId}`);
    if (!value) {
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to read live snapshot', 500, error);
  }
}
