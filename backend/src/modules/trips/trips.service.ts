...
export async function listTrips(
  ...
  try {
    ...
    if (query.from && query.to) {
      filters.push(
        between(trips.startTime, new Date(query.from), new Date(query.to)),
      )
    }
...
export async function getTripReplay(
  app: FastifyInstance,
  tenantId: string,
  tripId: string,
) {
  try {
    const gps = await app.db
      ...
      .from(gpsEvents)
      .where(
        and(eq(gpsEvents.tripId, tripId), eq(gpsEvents.tenantId, tenantId)),
      )
      .limit(1000)
    const fuel = await app.db
      ...
      .from(fuelEvents)
      .where(
        and(eq(fuelEvents.tripId, tripId), eq(fuelEvents.tenantId, tenantId)),
      )
      .limit(1000)
    return { gps, fuel }
  } catch (error: unknown) {
