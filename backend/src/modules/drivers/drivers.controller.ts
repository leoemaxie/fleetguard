...
export async function driverScoreController(
  request: FastifyRequest<{ Params: Params }>,
  reply: FastifyReply,
): Promise<void> {
  const score = await getDriverScore(
    request.server,
    request.tenantId,
    request.params.driverId,
  )
...
export async function driverScoreHistoryController(
  ...
  const history = await getDriverScoreHistory(
    request.server,
    request.tenantId,
    request.params.driverId,
  )
...
export async function computeDriverScoreController(
  ...
  const score = await computeDriverScore(
    request.server,
    request.tenantId,
    request.params.driverId,
  )
