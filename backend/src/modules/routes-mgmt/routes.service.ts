  ...
  try {
    const [route] = await app.db
      ...
      .values({
        tenantId,
        name: payload.name,
        corridorGeom: geomFromGeoJson(
          JSON.stringify(payload.corridorGeomGeoJson),
...
export async function deleteRouteStop(
  ...
  try {
    const [deleted] = await app.db
      ...
      .where(
        and(
          eq(approvedStops.id, stopId),
          eq(approvedStops.routeId, routeId),
          eq(approvedStops.tenantId, tenantId),
        ),
      )
      .returning()
    if (!deleted) {
