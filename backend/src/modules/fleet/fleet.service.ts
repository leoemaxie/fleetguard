...
import { alerts, fuelEvents, gpsEvents, vehicles } from '../../db/schema/index.js';
import { AppError, ErrorCode } from '../../lib/errors.js';

export async function fleetLive(app: FastifyInstance, tenantId: string) {
  try {
    const fleetVehicles = await app.db.select().from(vehicles).where(eq(vehicles.tenantId, tenantId));
    ...
    const rows = await Promise.all(
      fleetVehicles.map(async (vehicle) => {
        const [gps] = await app.db
          .select()
          .from(gpsEvents)
          .where(and(eq(gpsEvents.tenantId, tenantId), eq(gpsEvents.vehicleId, vehicle.id)))
          .limit(1);

        const [fuel] = await app.db
          .select()
          .from(fuelEvents)
          .where(and(eq(fuelEvents.tenantId, tenantId), eq(fuelEvents.vehicleId, vehicle.id)))
          .limit(1);

        const openAlerts = await app.db
          .select({ id: alerts.id })
          .from(alerts)
          .where(and(eq(alerts.tenantId, tenantId), eq(alerts.vehicleId, vehicle.id), isNull(alerts.resolvedAt)));
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to fetch fleet live', 500, error);
  }
}
...
export async function fleetSummary(app: FastifyInstance, tenantId: string) {
  try {
    ...
    throw new AppError(ErrorCode.INTERNAL_ERROR, 'Failed to fetch fleet summary', 500, error);
  }
}
