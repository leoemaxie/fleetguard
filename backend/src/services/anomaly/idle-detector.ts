...
export async function detectIdleExcess(
  db: DrizzleClient,
  input: IdleDetectionInput,
): Promise<boolean> {
  const gps = await db
    .select()
    .from(gpsEvents)
    .where(
      and(
        eq(gpsEvents.tenantId, input.tenantId),
        eq(gpsEvents.vehicleId, input.vehicleId),
      ),
    )
  ...
  const fuel = await db
    ...
    .where(
      and(
        eq(fuelEvents.tenantId, input.tenantId),
        eq(fuelEvents.vehicleId, input.vehicleId),
      ),
    )
