...
export function computeDriverWeeklyScore(trips: TripLike[], alerts: AlertLike[]): DriverScore {
  const routeCompliancePct =
    trips.length === 0
      ? 100
      : trips.reduce((sum, trip) => sum + Number(trip.complianceScore ?? 100), 0) / trips.length;
  ...
  const actualFuel = trips.reduce((sum, trip) => sum + Number(trip.totalFuelUsedLitres ?? 0), 0);
  const expectedFuel = distance * 0.18;
  const overRatio = expectedFuel === 0 ? 0 : Math.max(0, (actualFuel - expectedFuel) / expectedFuel);
  ...
  return {
    totalScore: Math.max(0, Math.min(100, routeComplianceScore + fuelScore + alertScore + stopScore)),
